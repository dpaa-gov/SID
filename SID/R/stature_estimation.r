stature_estimate <- function(reference, case, prediction_interval) {
    ...
    pi_list <- list()
    ref_list <- list()
    stats_df <- data.frame(PI = 0, measurements = "A", value = 0, `point estimate` = 0, lower = 0, upper = 0, n = 0, slope = 0, intercept = 0, `R²` = 0, check.names = FALSE)
    l <- 1
    for (i in seq_along(m_names)) { # for every measurement used
        ...
        for (j in seq_len(ncol(c_i))) { # for every combination at i index
            ...
            stats_df <- rbind(stats_df, data.frame(PI = round(pm1[1] - pm1[2], 2), measurements = paste(c_i[, j], collapse = " "), value = cf1$Measurements, `point estimate` = pm1[1], lower = pm1[2], upper = pm1[3], n = nrow(df1), slope = round(stats_lm1$coefficients[2], 5), intercept = round(stats_lm1$coefficients[1], 2), `R²` = round(stats_lm1$r.squared, 3), check.names = FALSE))
            l <- l + 1
        }
    }
